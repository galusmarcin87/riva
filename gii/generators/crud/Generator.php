...
class Generator extends \mootensai\enhancedgii\BaseGenerator
{
    ...
    public $nsModel = 'app\models\mgcms\db';
    public $nsSearchModel = 'app\models\mgcms\db';
    public $generateSearchModel = true;
    public $searchModelClass;
    public $generateQuery = true;
    public $queryNs = 'app\models\mgcms\db';
    public $queryClass;
    ...
    public $deletedBy = '';
    public $deletedAt = '';
    public $nsController = 'app\modules\backend\controllers\mgcms';
    public $controllerClass;
    public $pluralize = true;
    public $loggedUserOnly;
    public $expandable= true;
    public $cancelable= true;
    public $saveAsNew= true;
    public $pdf= true;
    public $viewPath = '@app/modules/backend/views/mgcms';
    public $baseControllerClass = '\app\components\mgcms\MgCmsController';
                    ...
                    if ($rel[self::REL_IS_MULTIPLE] && isset($rel[self::REL_TABLE]) && !in_array($name, $this->skippedRelations)) {
                        ...
                        if ($this->expandable) {
                          ...
                          $files[] = new CodeFile("$viewPath/_data{$rel[self::REL_CLASS]}.php", $this->render("views/_datarefmany.php", [
                                'relName' => $name,
                                'relations' => isset($relations[$tableName]) ? $relations[$tableName][$name] : [],
                                'modelClass' => str_replace($modelClassName, $relations[$tableName][$name][1], $this->modelClass),
                            ]));
                        }
...
//            $modelRel = $rel[2] ? lcfirst(Inflector::pluralize($rel[1])) : lcfirst($rel[1]);
            $output = "[
                ...
                'filterInputOptions' => ['placeholder' => ".$this->generateString(ucwords(Inflector::humanize($rel[5]))).", 'id' => '$id']
            ],\n";
            return $output;
        } else {
            return "'$attribute" . ($format === 'text' ? "" : ":" . $format) . "',\n";
...
//            $pk = empty($this->tableSchema->primaryKey) ? $this->tableSchema->getColumnNames()[0] : $this->tableSchema->primaryKey[0];
    ...
    public function generateActiveField($attribute, $fk, $tableSchema = null, $relations = null, $isTree = false)
    {
        ...
        if (in_array($attribute, $this->hiddenColumns)) {
            return "\$form->field12md($model, '$attribute', ['template' => '{input}'])->textInput(['style' => 'display:none']);";
        }
        $placeholder = Inflector::humanize($attribute, true);
        if ($tableSchema === false || !isset($tableSchema->columns[$attribute])) {
            if (preg_match('/^(password|pass|passwd|passcode)$/i', $attribute)) {
                return "\$form->field12md($model, '$attribute')->passwordInput()";
            } else if (in_array($attribute, $this->hiddenColumns)) {
                return "\$form->field($model, '$attribute')->hiddenInput()";
            } else {
                return "\$form->field12md($model, '$attribute')";
            }
        }
        $column = $tableSchema->columns[$attribute];
        if ($column->phpType === 'boolean' || $column->dbType === 'tinyint(1)') {
            return "\$form->field12md($model, '$attribute')->checkbox()";
        } elseif ($column->type === 'text' || $column->dbType === 'tinytext') {
            return "\$form->field12md($model, '$attribute')->textarea(['rows' => 6])";
        } elseif ($column->dbType === 'date') {
            return "\$form->field12md($model, '$attribute')->widget(\\kartik\\datecontrol\\DateControl::classname(), [
        'type' => \\kartik\\datecontrol\\DateControl::FORMAT_DATE,
        'saveFormat' => 'php:Y-m-d',
        'ajaxConversion' => true,
        'options' => [
            'pluginOptions' => [
                'placeholder' => Yii::t('app'," . $this->generateString('Choose ' . $placeholder) . "),
                'autoclose' => true
            ]
        ],
    ]);";
        ...
        'options' => [
            ...
            return "\$form->field($model, '$attribute')->datePicker();";
        } elseif (array_key_exists($column->name, $fk)) {
            $rel = $fk[$column->name];
            $labelCol = $this->getNameAttributeFK($rel[3]);
            $humanize = Inflector::humanize($rel[3]);
//            $pk = empty($this->tableSchema->primaryKey) ? $this->tableSchema->getColumnNames()[0] : $this->tableSchema->primaryKey[0];
        ...
        'pluginOptions' => [
            ...
            if (is_array($column->enumValues) && count($column->enumValues) > 0) {
                ...
                return "\$form->field12md($model, '$attribute')->dropDownList("
                . preg_replace("/\n\s*/", ' ', VarDumper::export($dropDownOptions)) . ", ['prompt' => ''])";
            } elseif ($column->phpType !== 'string' || $column->size === null) {
                return "\$form->field12md($model, '$attribute')->$input(['placeholder' => ".'$model->getAttributeLabel'."('$attribute')])";
            } else {
                return "\$form->field12md($model, '$attribute')->$input(['maxlength' => true, 'placeholder' => ".'$model->getAttributeLabel'."('$attribute')])";
            }
        }
    }
